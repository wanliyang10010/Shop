...
<script type="text/javascript" src="<%=basePath%>js/security/role/searchRole.js"></script>
...
<link rel="stylesheet" type="text/css" href="<%=basePath%>css/security/role/searchRole.css">
...
<body>
	...
	<div id="fanhui">
	 ...
	 <br/>
		...
		<form action="searchRoleAction_!search.action" method="post" id="myform">
				...
				<tr>
					...
					<td>
						<select id="selectId" name="selectId">
						  ...
						  <option value ="roleName"   <c:if test="${'roleName' eq selectId}">selected</c:if>>角色名称</option>
						  <option value ="description" <c:if test="${'description' eq selectId}">selected</c:if>>角色描述</option>
						  <option value ="allRole" <c:if test="${'allRole' eq selectId}">selected</c:if>>查询所有角色</option>
					    </select>
					</td>
				</tr>
				<tr>
			...
			<tbody>
				<s:iterator value="page.list" status="s">
					<tr onmouseover="over_color(this)" onmouseout="remove_color(this)"  class="tr-color cursor">
						...
						<td align="center" class="col2">${roleName}</td>
