...
public class DisputeFileAction extends BaseAction<DisputeFile> {
	/**
	 * 
	 */
	...
	public String save() {
		...
		if(!json.trim().equals(""))
		{
			...
			list.add(model);
			request.put("filecount", list.size());
		}
		else
		{
			int i=1;
	...
	public String cancelfile()
	{
		...
		request.put("DisputeFileList", null);
		request.put("json", json);
		return "listdelete";
	}
	...
	{
		String disputeId=ServletActionContext.getRequest().getParameter("disputeId");
