...
Showbo.Msg={  
    ...
    InitMsg:function(width){  
      ...
      html='<div class="top"><div class="right"><div class="msgtitle" id="dvMsgTitle"></div></div></div>'+  
    ...
    confirm:function(msg,yesfn,nofn,argData,width){  
      //fn为回调函数，参数和show方法的一致  
      this.show({buttons:{yes:'确认',no:'取消'},msg:msg,title:'提示',yesFunc:yesfn,noFunc:nofn,argData:argData,width:width});  
    },  
    ...
    show:function(cfg){  
      ...
      if(cfg.wait){  
        ...
        if(cfg.buttons.yes){  
          this.dvBtns.appendChild(this.createBtn(cfg.argData,cfg.buttons.yes,cfg.yesFunc));  
          if(cfg.buttons.no)this.dvBtns.appendChild(document.createTextNode('　'));  
        }  
        if(cfg.buttons.no)this.dvBtns.appendChild(this.createBtn(cfg.argData,cfg.buttons.no,cfg.noFunc));  
      }  
...
window.confirm = function(content,yes,no,argData,width){
	Showbo.Msg.confirm(content,yes,no,argData,width);
}
